...
func AddCommentHandler(db *sql.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		_, err := db.Exec(
			"INSERT INTO fiboblog.comments (post_id, user_id, content) VALUES ($1, $2, $3)",
...
func GetCommentsHandler(db *sql.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		rows, err := db.Query("SELECT id, post_id, user_id, content FROM fiboblog.comments WHERE post_id = $1", postID)
		if err != nil {
...
func DeleteCommentHandler(db *sql.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		_, err := db.Exec("DELETE FROM fiboblog.comments WHERE id = $1", commentID)
		if err != nil {
